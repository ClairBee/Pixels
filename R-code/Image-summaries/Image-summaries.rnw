...
print(xtable(img.summ[order(img.summ$batch),-c(1:2)], 
             align = "l|cc|ccccc|c",
...
Spots visibly worsened in images from 16-03-14.
...
text(rep(1.8, 3), c(7450, 22500, 53000), labels = c("Black batch", "Grey batch", "White batch"))
...
print(xtable(xml.summ[order(xml.summ$batch), t1], 
...
<<xtable-leftovers, results = 'asis', fig.pos = "!h">>=
ignore <- c("date", "batch", "ImageNotes", "ImageNotes.1", "TimeAcquired",
            "Time", "Date", ".attrs.FileName", ".attrs.ImageNotesOnlyValid", "XrayCurrentUsed",
            "WireSweep.Active", "CameraProperties.defaultImagingMode",
            "SavedImageProfile.FilePath", "ImageDetector")
...
if (length(leftover) > 0) {
    ...
    print(xtable(xml.summ[, colnames(xml.summ) %in% leftover], 
             ...
             caption = "Unlisted attributes of image data"),
          caption.placement = "top", 
          size = "footnotesize", 
          include.colnames = F, 
...
@
%              caption = paste0("Unlisted attributes of image data :", c(leftover))),
