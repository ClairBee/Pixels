...
\footnotesize{The black image shows a convex, roughly circular shape across the whole panel, with values lower than the mean in the centre, and higher at the edges of the panel. The grey and white images show a concave, roughly circular shape, having higher values in the centre and lower at the edges; there are a number of large dim flecks in the lower half of the panel, which are the shadows of specks of tungsten on the detector's beryllium window (see \nb{ref}). In all three images, discontinuities across the edges of the subpanels can be discerned quite clearly.}}
...
\begin{figure}[!ht]
\caption{Gaussian spot models fitted both with (solid line, square) and without (dashed line, triangle) constraints on the possible values of $x_0$ and $y_0$. \\
...
\sftriple{pwm-image-141009-white}{White pixelwise mean image}
%
\sftriple{sc-image-141009-white}{White shading-corrected image}
%
\sftriple{spots-overplotted}{Shift in apparent position}
